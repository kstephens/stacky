...
void s_T(T,_resize)(s_T(T,)* self, size_t s)
{
...
void s_T(T,_init)(s_T(T,)* self, size_t s)
{
  self->b = self->p = self->e = 0;
  self->s = 0;
  s_T(T,_resize)(self, s);
}
void s_T(T,_push)(s_T(T,)* self, s_v v)
{
  if ( self->p >= self->e )
    s_T(T,_resize)(self, self->s * 3 / 2 + 3);
  *(self->p ++) = T_unbox(v);
}
s_v s_T(T,_pop)(s_T(T,)* self)
{
